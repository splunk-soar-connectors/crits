...
class CritsConnector(BaseConnector):
    ...
    def initialize(self):
        ...
        self._params = {"username": config[CRITS_JSON_USERNAME], "api_key": config[CRITS_JSON_API_KEY]}
    ...
    def _process_html_response(self, response, action_result):
        ...
        if error_text.strip():
            message = f"Error from server: Status Code: {status_code}. Data from server:\n{error_text}\n"
        else:
    ...
    def _process_json_response(self, r, action_result):
        ...
        try:
            ...
            return RetVal(
                action_result.set_status(phantom.APP_ERROR, "Unable to parse response as JSON", self._get_error_message_from_exception(e)), None
            )
    ...
    def _make_rest_call(self, endpoint, action_result, params=None, data=None, headers=None, method="get", files=None, real_data=None):
        ...
        try:
            response = request_func(
                url, params=params, json=data, headers=headers, verify=self._verify, files=files, data=real_data, timeout=self._timeout
            )
        except Exception as e:
            error_message = self._get_error_message_from_exception(e)
            # Set the action_result status to error, the handler function will most probably return as is
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR, "Error connecting: {}".format(error_message.replace(self._params["api_key"], "<api_key>"))
                ),
                None,
            )
    ...
    def _get_resource(self, param):
        ...
        endpoint = f"/api/v1/{resource}/{res_id}/"
    ...
    def _update_resource(self, param):
        ...
        if "success" not in msg.lower():
            ...
            return action_result.set_status(phantom.APP_ERROR, f"Unable to update resource: {msg}")
    ...
    def _create_resource(self, param):
        ...
        if vault_id:
            try:
                ...
                data.update({"upload_type": "file", "file_format": "raw"})
            except Exception as e:
                error_message = self._get_error_message_from_exception(e)
                return action_result.set_status(phantom.APP_ERROR, f"Error reading file: {error_message}")
        else:
            files = None
        ...
        if not res_id:
            ...
            return action_result.set_status(phantom.APP_ERROR, "Unable to create resource: {}".format(response.get("message", "")))
    ...
    def _test_asset_connectivity(self, param):
        ...
        if total_count:
            self.save_progress("Got {} indicator{}".format(total_count, "s" if total_count > 1 else ""))
